...
本系统是安徽移动IP主体信息定位平台，基于 **FastAPI + Vue3 + Element Plus + SQLite** 技术栈构建。系统提供IP主体定位查询、数据源管理、主体信息模板管理、场景路径管理（可视化拖拽编排查询路径）、冲突工单全生命周期管理、批量查询与人工修正、敏感信息审批、操作日志、安全配置、用户管理及数据导出等完整功能。
...
- **路径回放**：`GET /api/query/tasks/{task_id}/path` — 返回任务执行路径快照（`path`：任务创建时所执行的场景路径的节点/连线/分支配置）及逐步执行记录（`paths`：每步含节点ID、状态、详情、数据源）。页面以路径图形式回放：路径即配置的场景路径，执行过的节点带✓/✗状态徽标，未走的分支以虚线显示，下方时间线展示每步详细结果；未配置发布路径的任务回放默认流程（开始→数据源查询→数据融合→返回结果）
...
管理五类IP主体信息定位场景，支持场景的新增、编辑、删除、开启/关闭，并通过可视化拖拽构建器编排场景查询路径（含开始/执行/判断节点），配置查询流程：哪类数据去哪个数据源查、判断什么后走哪条分支。

**五类场景**：移网、家宽、专线、IDC、自有业务

**场景管理**：
- 新增/编辑场景：输入场景名称、场景类型、场景包含的IP地址范围（CIDR，逗号分隔，如 10.0.0.0/8,100.64.0.0/10）
- 场景创建后支持开启/关闭（关闭后该场景查询路由不生效）
- 删除未被引用的场景
- 查询时若未指定场景类型，系统按IP地址范围自动匹配场景

**可视化路径构建器**：
- **开始节点**（蓝色）：流程起点
- **执行节点**（绿色）：绑定查询数据源（双击节点配置查询数据源与查询目标，即"哪类数据去哪个数据源查"）；未绑定数据源的执行节点作为返回结果节点
- **判断节点**（橙色）：配置判断条件（是否命中），出边带分支标签（命中/未命中），实现"判断什么后该查哪里"的分支路由
- 拖拽组件工具栏中的节点到画布放置
- 拖动节点调整布局，从节点右侧锚点拖拽连线到目标节点
- 判断节点连线时选择分支标签（命中/未命中），同标签连线自动替换
- 右键节点/连线删除，删除后可重新连线
- 支持画布缩放（放大/缩小）、平移拖动、重置视图、清空画布
- 路径保存（draft）与发布生效（published）

**查询执行逻辑**：发布路径后，IP查询任务按路径遍历执行——执行节点查询其绑定的数据源，判断节点按"是否命中"走对应分支（命中→返回结果，未命中→可连到下一个执行节点继续查询其他数据源）。任务创建时保存所执行路径的快照（节点/连线/分支），路径回溯/回放即展示该配置的场景路径本身及每步详细执行结果。

**API**:
- 场景列表：`GET /api/scenes`
- 新增场景：`POST /api/scenes`（含 ip_range）
- 编辑场景：`PUT /api/scenes/{id}`
- 删除场景：`DELETE /api/scenes/{id}`
- 场景状态切换：`PUT /api/scenes/{id}/toggle`
- 路径列表：`GET /api/scenes/{id}/paths`
- 新增路径：`POST /api/scenes/{id}/paths`（请求体含 nodes/edges JSON字符串，节点含 data_source_id/condition）
